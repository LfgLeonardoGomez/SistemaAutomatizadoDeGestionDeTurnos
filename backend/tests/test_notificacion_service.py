...
async def _seed_paciente(db_session, telegram_chat_id: str | None = "12345"):
    paciente = Paciente(
        nombre="Juan",
        apellido="Perez",
...
class TestObtenerTurnosParaRecordar:
    ...
    async def test_turno_dentro_de_ventana_es_incluido(self, db_session):
        ...
        turnos = await obtener_turnos_para_recordar(db_session, horas_antes=24)
        assert len(turnos) == 1
...
class TestEnviarRecordatorioTelegram:
    ...
    def setup_method(self):
        from app.services.telegram_service import _reset_bot, _reset_state
        _reset_bot()
        _reset_state()
    ...
    async def test_paciente_sin_chat_id_retorna_true_y_no_envia(self, db_session):
        ...
        profesional = await _seed_profesional(db_session)
        paciente = await _seed_paciente(db_session, telegram_chat_id=None)
...
class TestMarcarRecordatorioEnviado:
    ...
    async def test_marca_recordatorio_enviado(self, db_session):
        ...
        await marcar_recordatorio_enviado(db_session, turno.id)
        await db_session.refresh(turno)
    ...
    async def test_marcar_turno_inexistente_no_falla(self, db_session):
        ...
        await marcar_recordatorio_enviado(db_session, 99999)
